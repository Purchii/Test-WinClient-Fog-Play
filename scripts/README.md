...
The `GameSessionCanary` quality gate validates the local dry-run canary readiness plan without starting or stopping a game session, requires a canary synthetic alias in the direct plan validator, and asserts that `-AllowClientLaunch`, `-AllowNetwork` and `-AllowAuth` are rejected.

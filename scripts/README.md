...
The `ProdMetadataSafety` quality gate fails if production test metadata suites drift away from their required classifications, synthetic-user allowlist aliases, targets or session/cleanup flags, or if prod-canary target region/game metadata is not allowlisted by the resource budget and allowed-games fixtures.

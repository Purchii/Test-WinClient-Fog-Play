...
The `QaDocsSafety` quality gate fails if the required `docs/qa/*.md` policy inventory changes without updating the gate, if core safety phrases, release/privacy M1.1 hardening notes, Post-M6 finding coverage summaries including detailed AppSmoke/BackendSmoke/WebViewBridge finding coverage wording, BackendSmoke/release/privacy runner hardening notes or release/privacy/AppSmoke report-only coverage notes are removed, or if installed artifact defaults in active QA docs lose the manual/explicit-plan and local-fixture autonomous verification boundary.

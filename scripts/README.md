...
The `GameSessionCanary` quality gate validates the local dry-run canary readiness plan without starting or stopping a game session, requires an allowlisted production canary synthetic alias, matching synthetic-user duration ceiling and bounded run-frequency budget in the direct plan validator, and asserts that `-AllowClientLaunch`, `-AllowNetwork` and `-AllowAuth` are rejected.

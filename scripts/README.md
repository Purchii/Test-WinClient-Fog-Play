...
The `SessionLogSafety` quality gate fails if guarded session-log branch entries lose their mode, branch, scope or core no-runtime/no-production safety notes, if codex branch entries describe a previous source/coordinator thread as still active, if the most recent codex branch entry loses thread lifecycle wording, or if the latest session-log codex branch stops matching the latest verification-memory codex branch.

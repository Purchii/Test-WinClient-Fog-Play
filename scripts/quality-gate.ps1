...
function Invoke-SessionLogSafetyGate {
    ...
    if ($latestEntry.Value -match 'Branch:\s+`codex/') {
        ...
        if ($entryText -notmatch [regex]::Escape('same thread') -and
            $entryText -notmatch [regex]::Escape('preserved as inactive/history-only')) {
            throw "session-log.md latest entry '$title' must state whether discovery selected-task delivery stayed in the same thread or delegated discovery was preserved inactive/history-only."
        }
        foreach ($requiredPhrase in @(
                'No installed client launch',
                'No WebView debug/CDP',
...
function Invoke-DecisionsLogSafetyGate {
    ...
    foreach ($requiredPhrase in @(
            ...
            'Planner may select one safe bounded follow-up quality gate, hardening item, feature slice, backlog item or milestone, and Builder/QA may complete that selected task in the same thread',
...
function Invoke-CodexPolicySafetyGate {
    ...
    foreach ($requiredPhrase in @(
            ...
            'current task thread creates the next separate Codex continuation thread via `create_thread` and hands off the next discovery cycle instead of stopping after the first completed task',
    ...
    foreach ($requiredPhrase in @(
            ...
            'Treat a bounded autonomous continuation thread as the correct place for source-of-truth discovery, Planner selection of one safe bounded follow-up task, and Builder/QA completion of that selected task',
            'Treat extended autonomous time, push permission and merge permission as execution permissions only',
            'create the next separate Codex continuation thread via `create_thread` and hand off the next discovery cycle instead of stopping after the first completed task',
...
function Invoke-HandoffProtocolSafetyGate {
    ...
    foreach ($requiredPhrase in @(
            ...
            'Planner may select one safe, bounded follow-up gate, hardening item, feature slice or backlog item and Builder/QA may implement and verify that selected task in the same thread',
...
function Invoke-QaDocsSafetyGate {
    ...
    $runnerHardeningPhrasesByDoc = @{
        'app-webview-smoke.md' = @('Post-M6 guard hardening added', 'unsafe runtime input paths', 'missing `-DryRun`', '`-AllowClientLaunch`', '`-AllowWebViewDebugPort`')

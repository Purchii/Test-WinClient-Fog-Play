...
- `QualityGatesDocsScopeSafety` now requires the `PowerShellStructuredSyntaxSafety` summaries in quality-gates docs and scripts README to preserve PowerShell parser coverage;
- the guarded summary contract keeps `scripts/*.ps1`, `src/TestFramework/**/*.ps1`, `src/TestFramework/**/*.psm1`, `[System.Management.Automation.Language.Parser]::ParseFile(...)` and no-import/no-execute wording visible across both docs;
- the task stayed local/static and did not change runtime, client, auth, network, WebView, CI/CD, dependency, fake/replay execution, hardware probing or game-session behavior.
...
- `QualityGatesDocsScopeSafety` now requires the `NonProdFoundation` summaries in quality-gates docs and scripts README to preserve explicit missing required contract property coverage;
- the guard now prevents the NonProdFoundation summary contract from silently regressing that coverage into only generic structural component finding wording;
- the task stayed local/static and did not change runtime, client, auth, network, WebView, CI/CD, dependency, fake/replay execution, hardware probing or game-session behavior.
...
- `QualityGatesDocsScopeSafety` now requires the `AppSmoke`, `BridgeContract` and `BackendSmoke` summaries in quality-gates docs and scripts README to preserve missing `-DryRun` rejection coverage;
- the guarded runner summaries now explicitly include missing `-DryRun` alongside unsafe runtime input paths and dangerous allow-flag rejection coverage;
- the task stayed local/static and did not change runtime, client, auth, network, WebView, CI/CD, dependency or game-session behavior.
...
- `QualityGatesDocsScopeSafety` now requires the `ActiveVerificationCommandSafety` summaries in `docs/context/engineering/quality-gates.md` and `scripts/README.md` to preserve the same core active verification command evidence fragments;
- the guarded summary contract covers active/current verification command evidence, active-run Last verification, latest verification-memory Commands block, local/static quality-gate or git checks, command dedupe, installed-client and user runtime path bans, URL/localhost/debug/CDP bans, direct runner bans, auth/network/session/update/dependency bans, TestabilityGaps production-action/runtime-user-data allow flags and CI command bans;
- the task stayed local/static and did not change runtime, client, auth, network, WebView, CI/CD, dependency or game-session behavior.
...
- `QualityGatesDocsScopeSafety` now requires the `SessionLogSafety` summaries in `docs/context/engineering/quality-gates.md` and `scripts/README.md` to preserve the same core metadata, lifecycle and latest parity fragments;
- the guarded summary contract covers `docs/context/governance/session-log.md`, codex branch entries, `Mode:`, `Branch:`, `Scope:`, `Safety:`, core no-runtime/no-production safety notes, previous source/coordinator lifecycle wording, same-thread selected-task delivery, inactive delegated discovery and latest verification-memory title/branch parity;
- the task stayed local/static and did not change runtime, client, auth, network, WebView, CI/CD, dependency or game-session behavior.
...
- executor-policy now matches AGENTS.md, context-protocol and git-workflow wording that a bounded autonomous continuation thread may select a follow-up gate, hardening item, feature slice or backlog item;
- `CodexPolicySafety` now preserves the executor-policy continuation discovery phrase including `feature slice`;
- the task stayed local/static and did not change runtime, client, auth, network, WebView, CI/CD, dependency or game-session behavior.
...
- active command evidence must not repeat a command within one source block;
...
- `GameSessionCanary` summary parity requires exact non-duplicated canary suite metadata wording alongside readiness signal, synthetic alias, budget and runtime-path guard coverage;
...
- `Assert-CommandRejected` must be defined exactly once;
...
- `TestabilityGaps` quality gate asserts missing `-DryRun`, unsafe next-safe-step text, `-AllowProductionAction`, `-AllowCredentials` and `-AllowRuntimeUserData` rejection.
...
- AppSmoke guard hardening wording must keep unsafe runtime input path, missing `-DryRun`, `-AllowClientLaunch` and `-AllowWebViewDebugPort` rejection coverage visible in the QA doc;

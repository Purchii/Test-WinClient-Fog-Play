...
The `GameSessionCanary` scope validates the local dry-run canary readiness plan without starting or stopping a game session, requires a canary synthetic alias in direct plan validation, and asserts dangerous runtime allow flags are rejected.

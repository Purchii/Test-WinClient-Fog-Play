...
The `QaDocsSafety` scope statically checks the `docs/qa/*.md` policy inventory, core safety phrases, Post-M6 finding coverage summaries and release/privacy runner hardening notes so QA policy docs cannot be added, removed or weakened silently. It also keeps installed artifact defaults in active QA docs bounded to manual/explicit-plan reads while autonomous verification remains committed local fixtures only.

...
The `GameSessionCanary` scope validates the local dry-run canary readiness plan without starting or stopping a game session, requires an allowlisted canary synthetic alias and matching synthetic-user duration ceiling in direct plan validation, and asserts dangerous runtime allow flags are rejected.

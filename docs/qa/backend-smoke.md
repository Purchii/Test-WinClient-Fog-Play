...
Post-M6 guard hardening added `BackendSmoke` quality gate assertions that missing `-DryRun`, `-AllowNetwork` and `-AllowAuth` are rejected before any backend network or auth action can occur. The direct validator also returns a `dry-run-flag-required` fail finding without the dry-run flag.
